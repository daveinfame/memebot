/* MEMEBOT v3.4.5 — BSC FIX CON BLOCKSCOUT */
...
const BANNER = `\n MEMEBOT v3.4.5 BSC FIX BS_KEY:${BLOCKSCOUT_API_KEY? "SI ✅":"NO ❌"}`;
function loadStateFile() { try { if (fs.existsSync(STATE_FILE)) { const s = JSON.parse(fs.readFileSync(STATE_FILE, "utf8")); if (s && typeof s.reserva === "number") { if (!s.customWallets) s.customWallets = []; if (!s.tgSubs) s.tgSubs = []; if (!s.evmWallets) s.evmWallets = {}; return s; } } } catch (e) { say.warn(`state.json corrupto (${e.message})`); } return { version: 3.45, reserva: RESERVA_INICIAL, usdc: 0, startedAt: Date.now(), positions: {}, evmPositions: {}, pnl: { solana: { pnlSol: 0, wins: 0, losses: 0, trades: 0, usdc: 0 }, base: { pnl: 0, wins: 0, losses: 0, trades: 0 }, bsc: { pnl: 0, wins: 0, losses: 0, trades: 0 }, robinhood: { pnl: 0, wins: 0, losses: 0, trades: 0 }, hyperevm: { pnl: 0, wins: 0, losses: 0, trades: 0 }, hyperliquid: { pnl: 0, wins: 0, losses: 0, trades: 0 } }, byWallet: {}, wallets: {}, tgSubs: [], customWallets: [], evmWallets: {} }; }
function migrateState(s){ if(!s.evmPositions) s.evmPositions={}; if(!s.pnl) s.pnl={ solana:{pnlSol:0,wins:0,losses:0,trades:0,usdc:0}, base:{pnl:0,wins:0,losses:0,trades:0}, bsc:{pnl:0,wins:0,losses:0,trades:0}, robinhood:{pnl:0,wins:0,losses:0,trades:0}, hyperevm:{pnl:0,wins:0,losses:0,trades:0}, hyperliquid:{pnl:0,wins:0,losses:0,trades:0}}; if(!s.byWallet) s.byWallet={}; for(const k of ["base","bsc","robinhood","hyperevm","hyperliquid","solana"]){ if(!s.pnl[k]) s.pnl[k]={pnl:0,pnlSol:0,wins:0,losses:0,trades:0,usdc:0}; } s.version=3.45; return s; }
...
async function pollEvmWallet(w) {
  ...
  try {
    ...
    if (chain === "robinhood") {
      ...
      try {
        // FIX: Usamos Blockscout de BSC igual que Base/Robinhood, no BscScan
        const url = `https://bsc.blockscout.com/api/v2/addresses/${w.address}/token-transfers?${apiKeyQuery}items_count=20`;
        const { data } = await http.get(url, {timeout:10000, headers});
        if(data?.items?.length){
          transfers = data.items.map(it=>({ hash: it.transaction_hash, timeStamp: Math.floor(new Date(it.timestamp).getTime()/1000), token: it.token?.address, symbol: it.token?.symbol||"TOKEN", isBuy: (it.to?.hash||"").toLowerCase()===w.address.toLowerCase() }));
          console.log(`[BSC BS KEY] ${w.alias} ${transfers.length} transfers ✅`);
        } else {
          console.log(`[BSC BS KEY] ${w.alias} 0 transfers (wallet sin movimientos en Blockscout BSC)`);
        }
      } catch(e){ console.log(`[BSC BS ERR] ${w.alias} status:${e.response?.status} ${e.message.slice(0,100)}`); }
    }
...
async function startTelegram() {
  ...
  tgBot.command("start", async (ctx) => {
    ...
    ctx.reply(`🟢 <b>MEMEBOT v3.4.5</b> - BS_KEY:${BLOCKSCOUT_API_KEY? "SI ✅":"NO ❌"} BSC FIX\n/estado · /pos · /wallets · /pnl · /top · /heartbeat`, { parse_mode: "HTML" });
  });
  tgBot.command("help", (ctx) => ctx.reply("<b>Comandos v3.4.5</b>\n/estado\n/pos\n/wallets\n/pnl [ALIAS]\n/top\n/heartbeat", { parse_mode: "HTML" }));
  ...
  tgBot.launch({ dropPendingUpdates: true }).then(() => say.ok("Telegram v3.4.5 KEY ✅ BSC FIX"));
