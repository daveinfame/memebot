/* MEMEBOT v3.4.4 — CON BLOCKSCOUT API KEY */
...
const BANNER = `\n MEMEBOT v3.4.4 BS_KEY:${BLOCKSCOUT_API_KEY? "SI ✅":"NO ❌"}`;
function loadStateFile() { try { if (fs.existsSync(STATE_FILE)) { const s = JSON.parse(fs.readFileSync(STATE_FILE, "utf8")); if (s && typeof s.reserva === "number") { if (!s.customWallets) s.customWallets = []; if (!s.tgSubs) s.tgSubs = []; if (!s.evmWallets) s.evmWallets = {}; return s; } } } catch (e) { say.warn(`state.json corrupto (${e.message})`); } return { version: 3.44, reserva: RESERVA_INICIAL, usdc: 0, startedAt: Date.now(), positions: {}, evmPositions: {}, pnl: { solana: { pnlSol: 0, wins: 0, losses: 0, trades: 0, usdc: 0 }, base: { pnl: 0, wins: 0, losses: 0, trades: 0 }, bsc: { pnl: 0, wins: 0, losses: 0, trades: 0 }, robinhood: { pnl: 0, wins: 0, losses: 0, trades: 0 }, hyperevm: { pnl: 0, wins: 0, losses: 0, trades: 0 }, hyperliquid: { pnl: 0, wins: 0, losses: 0, trades: 0 } }, byWallet: {}, wallets: {}, tgSubs: [], customWallets: [], evmWallets: {} }; }
function migrateState(s){ if(!s.evmPositions) s.evmPositions={}; if(!s.pnl) s.pnl={ solana:{pnlSol:0,wins:0,losses:0,trades:0,usdc:0}, base:{pnl:0,wins:0,losses:0,trades:0}, bsc:{pnl:0,wins:0,losses:0,trades:0}, robinhood:{pnl:0,wins:0,losses:0,trades:0}, hyperevm:{pnl:0,wins:0,losses:0,trades:0}, hyperliquid:{pnl:0,wins:0,losses:0,trades:0}}; if(!s.byWallet) s.byWallet={}; for(const k of ["base","bsc","robinhood","hyperevm","hyperliquid","solana"]){ if(!s.pnl[k]) s.pnl[k]={pnl:0,pnlSol:0,wins:0,losses:0,trades:0,usdc:0}; } s.version=3.44; return s; }
    ...
    if (chain === "robinhood") {
      try {
        ...
        if(data?.items?.length){
          ...
          console.log(`[ROBINHOOD BS KEY] ${w.alias} 0 transfers (vacio)`);
        }
      ...
      try {
        const bscUrl = `https://api.etherscan.io/v2/api?chainid=56&module=account&action=tokentx&address=${w.address}&sort=desc&page=1&offset=10${BLOCKSCOUT_API_KEY? `&apikey=${BLOCKSCOUT_API_KEY}` : ""}`;
        // Fallback a bscscan si no tienes etherscan key, pero intentamos bscscan directo
        const {data}=await http.get(`https://api.bscscan.com/api?module=account&action=tokentx&address=${w.address}&sort=desc`, {timeout:8000});
        if(data.result && Array.isArray(data.result)) transfers=data.result.slice(0,10).map(it=>({hash:it.hash,timeStamp:Number(it.timeStamp),token:it.contractAddress,symbol:it.tokenSymbol,isBuy:it.to?.toLowerCase()===w.address.toLowerCase()}));
        console.log(`[BSC] ${w.alias} ${transfers.length} transfers`);
      } catch(e){ console.log(`[BSC ERR] ${e.message.slice(0,80)}`); }
    }
...
async function startTelegram() {
  ...
  tgBot.command("start", async (ctx) => {
    ...
    ctx.reply(`🟢 <b>MEMEBOT v3.4.4</b> - BS_KEY:${BLOCKSCOUT_API_KEY? "SI ✅":"NO ❌"}\n/estado · /pos · /wallets · /pnl · /top · /heartbeat`, { parse_mode: "HTML" });
  });
  tgBot.command("help", (ctx) => ctx.reply("<b>Comandos v3.4.4</b>\n/estado\n/pos\n/wallets\n/pnl [ALIAS]\n/top\n/heartbeat", { parse_mode: "HTML" }));
  ...
  tgBot.launch({ dropPendingUpdates: true }).then(() => say.ok("Telegram v3.4.4 KEY ✅"));
