/* ============================================================
   MEMEBOT v3 — multi-chain: Solana + BASE + BSC + Robinhood
   NUEVO: /add ALIAS WALLET RED MONTO
   ============================================================ */
...
async function initDB() { if (!pool) return; try { await pool.query(` CREATE TABLE IF NOT EXISTS bot_state ( id INT PRIMARY KEY, data JSONB NOT NULL ); `); const res = await pool.query("SELECT data FROM bot_state WHERE id = 1"); if (res.rows.length === 0) { const initial = { version: 3, reserva: RESERVA_INICIAL, usdc: 0, startedAt: Date.now(), positions: {}, wallets: {}, tgSubs: [], customWallets: [], evmWallets: {} }; await pool.query("INSERT INTO bot_state (id, data) VALUES (1, $1)", [initial]); console.log("✅ Postgres conectado - tabla bot_state creada"); } else { console.log("✅ Postgres conectado - tablas listas"); } } catch (e) { console.log(`❌ Error initDB: ${e.message}`); } }
...
const BANNER = `\n ███╗ ███╗███████╗███╗ ███╗███████╗██████╗ ██████╗ ████████╗\n ████╗ ████║██╔════╝████╗ ████║██╔════╝██╔══██╗██╔═══██╗╚══██╔══╝\n ██╔████╔██║█████╗ ██╔████╔██║█████╗ ██████╔╝██║ ██║ ██║\n ██║╚██╔╝██║██╔══╝ ██║ ████║██╔══╝ ██║ ██║██║ ██║ ██║\n ██║ ╚═╝ ██║███████╗██║ ╚═╝ ██║███████╗██████╔╝╚██████╔╝ ██║\n v3.1 multi-chain: solana + base + bsc + robinhood + hyperevm (evm shortcut + pump pre-grad 36k)`;
function loadStateFile() { try { if (fs.existsSync(STATE_FILE)) { const s = JSON.parse(fs.readFileSync(STATE_FILE, "utf8")); if (s && typeof s.reserva === "number") { if (!s.customWallets) s.customWallets = []; if (!s.tgSubs) s.tgSubs = []; if (!s.evmWallets) s.evmWallets = {}; s.customWallets = s.customWallets.map(w => { if (w.chain) return w; return { address: w.address, alias: w.alias, chain: "solana", tradeAmount: String(w.tradeSol || DEFAULT_TRADE_SOL), type: "solana" }; }); return s; } } } catch (e) { say.warn(`state.json corrupto (${e.message})`); } return { version: 3, reserva: RESERVA_INICIAL, usdc: 0, startedAt: Date.now(), positions: {}, wallets: {}, tgSubs: [], customWallets: [], evmWallets: {} }; }
...
async function applyEvent(w, ev) { const ws = walletState(w.address, "solana"); const solUsd = await getSolUsd(); const symbol = await getTicker(ev.mint); if (ev.type === "buy") { if (SNAPSHOT_ON && ws.snapshotIgnored.includes(ev.mint)) { ws.stats.ignored++; await saveState(); return notify({ title: `🚫 R0 · ${w.alias} operó $${esc(symbol)} (ya lo tenía) → IGNORADO`, lines: [`Contrato: <code>${ev.mint}</code>`], color: YELLOW, hash: ev.txHash, chain: "solana" }); } if (ws.dusted.includes(ev.mint)) { ws.dusted = ws.dusted.filter((m) => m!== ev.mint); await saveState(); say.info(`${w.alias} habilitó $${symbol}`); } if (state.positions[ev.mint]) { ws.stats.ignored++; await saveState(); return notify({ title: `🚫 R2 · ${w.alias} promedió $${esc(symbol)} → IGNORADO`, lines: [`El bot mantiene su entrada original.`], color: YELLOW, hash: ev.txHash, chain: "solana" }); } const size = Math.min(Number(w.tradeAmount), ev.solAmount); if (state.reserva < size) { return notify({ title: `⚠️ R1 · RESERVA BAJA — no se copió $${esc(symbol)}`, lines: [`${w.alias} compró ${ev.solAmount.toFixed(4)} SOL${usd(ev.solAmount, solUsd)}`, `Reserva: ${state.reserva.toFixed(4)} SOL`], color: RED, hash: ev.txHash, chain: "solana" }); } const entryPrice = ev.solAmount / ev.tokenAmount; state.positions[ev.mint] = { wallet: w.address, alias: w.alias, symbol, chain: "solana", entryPrice, amountSol: size, tokenAmount: size / entryPrice, openedAt: Date.now() }; state.reserva -= size; ws.stats.copies++; await saveState(); return notify({ title: `🟢 R1 · ${w.alias} COMPRÓ $${esc(symbol)} — BOT ENTRÓ [SOL]`, lines: [`${w.alias} pagó: <b>${ev.solAmount.toFixed(4)} SOL</b>${usd(ev.solAmount, solUsd)}`, `🤖 Bot: <b>${size.toFixed(4)} SOL</b> @ ${entryPrice.toExponential(3)}`, `Contrato: <code>${ev.mint}</code>`], color: GREEN, hash: ev.txHash, chain: "solana" }); } if (ev.type === "dust") { if (ANTI_DUST_ON &&!ws.dusted.includes(ev.mint)) ws.dusted.push(ev.mint); ws.stats.dust++; await saveState(); return; } if (ws.snapshotIgnored.includes(ev.mint) && ev.postBalance <= 0) { ws.snapshotIgnored = ws.snapshotIgnored.filter((m) => m!== ev.mint); await saveState(); } const open = state.positions[ev.mint]; if (!open) return; const exitPrice = ev.tokenAmount > 0? ev.solAmount / ev.tokenAmount : open.entryPrice; const proceeds = open.amountSol * (exitPrice / open.entryPrice); const pnlSol = proceeds - open.amountSol; const pnlPct = (exitPrice / open.entryPrice - 1) * 100; let r5Line; if (pnlSol > 0 && AUTO_USDC_ON) { const gain = pnlSol * (solUsd?? 0); state.reserva += open.amountSol; state.usdc += gain; ws.stats.usdcSecured += gain; r5Line = `💰 R5 GANANCIA: <b>+${pnlSol.toFixed(4)} SOL</b>`; } else { state.reserva += open.amountSol + pnlSol; r5Line = pnlSol >= 0? `💰 R5 GANANCIA: <b>+${pnlSol.toFixed(4)} SOL</b>` : `💸 R5 PÉRDIDA: <b>−${Math.abs(pnlSol).toFixed(4)} SOL</b>`; } delete state.positions[ev.mint]; ws.stats.pnlSol += pnlSol; await saveState(); const win = pnlSol >= 0; return notify({ title: `${win? "🟢" : "🔴"} R3 · ${w.alias} VENDIÓ $${esc(symbol)} — BOT CERRÓ`, lines: [`PnL: <b>${win? "+" : "−"}${Math.abs(pnlPct).toFixed(1)}%</b>`, r5Line, `Reserva: ${state.reserva.toFixed(4)} SOL`], color: win? GREEN : RED, hash: ev.txHash, chain: "solana" }); }
const seenEvmTx = new Set();
async function pollEvmWallet(w) { const chain = (w.chain || "base").toLowerCase(); const ws = walletState(w.address, chain); try { let txList = []; if (chain === "base") { try { const { data } = await http.get(`https://api.basescan.org/api?module=account&action=txlist&address=${w.address}&startblock=0&endblock=99999999&sort=desc`); if (data.result && Array.isArray(data.result)) txList = data.result.slice(0, 5); } catch {} } else if (chain === "bsc") { try { const { data } = await http.get(`https://api.bscscan.com/api?module=account&action=txlist&address=${w.address}&sort=desc`); if (data.result && Array.isArray(data.result)) txList = data.result.slice(0, 5); } catch {} } else if (chain === "robinhood") { try { const { data } = await http.get(`https://robinhoodchain.blockscout.com/api/v2/addresses/${w.address}/transactions?items_count=5`); if (data.items) txList = data.items.map(it => ({ hash: it.hash, timeStamp: new Date(it.timestamp).getTime()/1000, input: it.raw_input })); } catch {} } else if (chain === "hyperevm" || chain === "hyperliquid") { try { const { data } = await http.get(`https://www.hyperevmscan.io/api/v2/addresses/${w.address}/transactions?items_count=5`); if (data.items) txList = data.items.map(it => ({ hash: it.hash, timeStamp: new Date(it.timestamp).getTime()/1000, input: it.raw_input })); } catch {} } for (const tx of txList) { const hash = tx.hash; if (!hash || seenEvmTx.has(hash)) continue; const tsTx = Number(tx.timeStamp || 0) * 1000; if (tsTx && tsTx <= ws.lastBlockTime) continue; seenEvmTx.add(hash); const isSwap = tx.input && tx.input!== "0x" && tx.input.length > 10; if (!isSwap) { ws.lastBlockTime = Math.max(ws.lastBlockTime, tsTx || Date.now()); continue; } ws.lastBlockTime = Math.max(ws.lastBlockTime, tsTx || Date.now()); ws.stats.copies++; await saveState(); await notify({ title: `🟢 [${chain.toUpperCase()}] ${w.alias} hizo SWAP`, lines: [`Wallet: <code>${w.address.slice(0,6)}…${w.address.slice(-4)}</code>`, `Monto a copiar: <b>${w.tradeAmount} ${chain==='bsc'?'BNB':'ETH'}</b>`, `Tx: <code>${hash.slice(0,12)}…</code>`], color: CYAN, hash, chain }); } } catch (e) { say.warn(`EVM poll ${chain} ${w.alias} falló: ${e.message}`); } }
async function loopEvm() { const evmChains = ["base", "bsc", "robinhood", "hyperevm"]; for (const chain of evmChains) { const wallets = getWalletsByChain(chain); for (const w of wallets) { await pollEvmWallet(w); await new Promise(r => setTimeout(r, 600)); } } }
...
async function loop() { for (const w of getWalletsByChain("solana")) { await pollWallet(w); await new Promise((r) => setTimeout(r, 400)); } }
...
function expandChainInput(input){
  ...
  if(input.includes(",")){
    const out=[];
    for(const p of input.split(",")){ out.push(...expandChainInput(p)); }
    return [...new Set(out)];
  }
...
// Pump.fun direct - para tokens que aun NO graduan (no importa si es 5k, 10k, 36k mcap)
// Si Jupiter falla, compra directo bonding curve
async function pumpFunBuyDirect(mint, solAmount){
  try{
    say.info(`🔎 Token pre-grad ${mint.slice(0,6)}... mcap bajo, compra directa bonding curve`);
    const { data } = await http.post("https://pumpportal.fun/api/trade-local", {
    ...
    if(data) { say.ok(`✅ Tx Pump.fun pre-grad generada para ${mint.slice(0,6)}...`); return data; }
  }catch(e){ say.warn(`Pump.fun direct falló (normal si ya graduó): ${e.message}`); }
  return null;
}
...
function parseAddCommand(text) {
  ...
  if (parts.length < 2) return { error: "Uso: /add ALIAS WALLET RED MONTO - tip: usa 'evm' para base,bsc,robinhood,hyperevm" };
  ...
  if (isOldFormat) {
    wallet = parts[1];
    alias = parts[2] || `${wallet.slice(0,4)}…${wallet.slice(-4)}`;
    const maybeAmount = parts[3];
    return { wallets: [{ alias, address: wallet, chain: wallet.startsWith("0x")? "base" : "solana", tradeAmount: maybeAmount || (wallet.startsWith("0x")? DEFAULT_TRADE_EVM : String(DEFAULT_TRADE_SOL)) }], old: true };
  }
  alias = parts[1];
  wallet = parts[2];
  chainPart = parts[3] || "solana";
  amountPart = parts[4];
  if (!wallet) return { error: "Falta wallet. Uso: /add ALIAS WALLET RED MONTO" };
  const result = [];
  const expandedChains = expandChainInput(chainPart);
  for(const ch of expandedChains){
    result.push({ alias, address: wallet, chain: ch.toLowerCase().trim(), tradeAmount: amountPart || (ch.toLowerCase()==="solana"? String(DEFAULT_TRADE_SOL) : DEFAULT_TRADE_EVM) });
  }
  for (const w of result) {
    if (w.chain === "solana") { if (w.address.length < 32) return { error: `Wallet Solana invalida: ${w.address}` }; }
    else { if (!isAddress(w.address)) return { error: `Wallet EVM invalida: ${w.address}` }; }
    const validChains = ["solana", "base", "bsc", "robinhood", "hyperevm", "hyperliquid"];
    if (!validChains.includes(w.chain)) return { error: `Chain invalida ${w.chain}. Usa: solana, base, bsc, robinhood, hyperevm o 'evm' para todas` };
  }
  return { wallets: result };
}

async function startTelegram() { if (!TELEGRAM_TOKEN) { say.warn("sin TELEGRAM_BOT_TOKEN"); return; } tgBot = new Telegraf(TELEGRAM_TOKEN); tgBot.command("start", async (ctx) => { if (!state.tgSubs.includes(ctx.chat.id)) { state.tgSubs.push(ctx.chat.id); await saveState(); } const dbStatus = pool? "PostgreSQL Connected ✅" : "Local file"; ctx.reply(`🟢 <b>MEMEBOT v3.1 multi-chain</b> - DB: ${dbStatus}\nChains: solana, base, bsc, robinhood, hyperevm (999)\n\n<b>Nuevo /add:</b>\n<code>/add ALIAS WALLET RED MONTO</code>\nEj: <code>/add ballena1 0x1234... evm 0.01</code> = base+bsc+robinhood+hyperevm\n<code>/add ballena1 0x1234... all 0.01</code> = todas\n\n/estado · /pos · /wallets\n/remove ALIAS`, { parse_mode: "HTML" }); }); tgBot.command("help", (ctx) => ctx.reply("<b>Comandos v3.1</b>\n/estado\n/pos\n/wallets\n/add ALIAS WALLET RED MONTO (usa evm o all)\n/remove ALIAS", { parse_mode: "HTML" })); tgBot.command("add", async (ctx) => { const parsed = parseAddCommand(ctx.message.text); if (parsed.error) return ctx.reply(parsed.error); const added = []; for (const nw of parsed.wallets) { const exists = getAllWallets().some(w => w.address.toLowerCase() === nw.address.toLowerCase() && (w.chain||"solana").toLowerCase() === nw.chain.toLowerCase()); if (exists) { await ctx.reply(`⚠️ Ya existe ${nw.alias} en ${nw.chain}`); continue; } const obj = { address: nw.address, alias: nw.alias, chain: nw.chain, tradeAmount: String(nw.tradeAmount), type: nw.chain === "solana"? "solana" : "evm" }; state.customWallets.push(obj); added.push(obj); await snapshotWallet(obj); } await saveState(); if (added.length) { ctx.reply(`✅ Agregadas ${added.length}:\n` + added.map(a => `• <b>${esc(a.alias)}</b> ${a.chain} — <code>${a.address.slice(0,6)}…${a.address.slice(-4)}</code> — ${a.tradeAmount}`).join("\n"), { parse_mode: "HTML" }); } }); tgBot.command("remove", async (ctx) => { const parts = ctx.message.text.trim().split(/\s+/); const target = parts[1]; if (!target) return ctx.reply("Uso: /remove ALIAS o DIRECCION"); const before = state.customWallets.length; const chainFilter = parts[2]; if (chainFilter) { state.customWallets = state.customWallets.filter(w =>!((w.alias===target || w.address.toLowerCase()===target.toLowerCase()) && w.chain===chainFilter.toLowerCase())); } else { state.customWallets = state.customWallets.filter(w => w.address.toLowerCase()!== target.toLowerCase() && w.alias!== target); } if (before === state.customWallets.length) return ctx.reply(`No encontré ${target}`); await saveState(); ctx.reply(`🗑️ Eliminada: ${esc(target)}`); }); tgBot.command("estado", async (ctx) => { const solUsd = await getSolUsd(); const open = Object.values(state.positions); const dbInfo = pool? "🐘 Postgres" : "📁 Archivo"; const evmInfo = evmAccount? `EVM: ${evmAccount.address.slice(0,6)}…` : "EVM: solo lectura"; ctx.reply(`<b>📊 ESTADO - ${dbInfo}</b>\nReserva: <b>${state.reserva.toFixed(4)} SOL</b>${usd(state.reserva, solUsd)}\n${evmInfo}\nWallets: ${getAllWallets().length} total — SOL:${getWalletsByChain('solana').length} BASE:${getWalletsByChain('base').length} BSC:${getWalletsByChain('bsc').length} ROB:${getWalletsByChain('robinhood').length} HYPE:${getWalletsByChain('hyperevm').length}`, { parse_mode: "HTML" }); }); tgBot.command("pos", (ctx) => { const open = Object.values(state.positions); if (!open.length) return ctx.reply("Sin posiciones."); ctx.reply(open.map((p) => `• <b>$${esc(p.symbol)}</b> [${p.chain||'sol'}] — ${p.amountSol.toFixed(4)} SOL`).join("\n"), { parse_mode: "HTML" }); }); tgBot.command("wallets", (ctx) => { const all = getAllWallets(); if (!all.length) return ctx.reply("Sin wallets."); const byChain = {}; for (const w of all) { const c = w.chain || "solana"; if (!byChain[c]) byChain[c]=[]; byChain[c].push(w); } let msg = ""; for (const [chain, list] of Object.entries(byChain)) { msg += `<b>${chain.toUpperCase()} (${list.length}):</b>\n`; msg += list.map((w) => `• <b>${esc(w.alias)}</b> — ${w.tradeAmount} · ${w.address.slice(0,6)}…`).join("\n") + "\n\n"; } ctx.reply(msg, { parse_mode: "HTML" }); }); tgBot.command("reset", async (ctx) => { state.reserva = RESERVA_INICIAL; state.usdc = 0; state.positions = {}; for (const a of Object.keys(state.wallets)) { state.wallets[a].stats = { copies: 0, ignored: 0, dust: 0, pnlSol: 0, usdcSecured: 0 }; } await saveState(); ctx.reply(`🔄 Tesorería reiniciada: ${RESERVA_INICIAL} SOL`); }); try { await tgBot.telegram.deleteWebhook({ drop_pending_updates: true }); } catch {} try { const t = tgBot.token; if (t) await axios.get('https://api.telegram.org/bot' + t + '/deleteWebhook?drop_pending_updates=true'); say.ok("Webhook limpiado - fix 409"); } catch {} await new Promise(r => setTimeout(r, 2000)); tgBot.launch({ dropPendingUpdates: true }).then(() => say.ok("Telegram en vivo + Postgres + EVM + Hyperliquid ✅")); tgBot.catch((e) => say.err(`telegram: ${e.message}`)); }
...
console.log(BANNER);
const boot = async () => { await initDB(); if (pool) { const dbState = await loadStateDB(); if (dbState) { state = dbState; if (!state.evmWallets) state.evmWallets = {}; say.ok(`DB: PostgreSQL Connected ✅ - ${state.customWallets.length} wallets`); } } say.info(`RPC SOL: ${HELIUS_API_KEY? "Helius" : "publico"}`); say.info(`RPC EVM: BASE ${BASE_RPC} | BSC ${BSC_RPC} | ROBINHOOD ${ROBINHOOD_RPC} | HYPERLIQUID ${HYPERLIQUID_RPC} (999)`); say.info(`Wallets: ${getAllWallets().length} total`); say.info(`EVM account: ${evmAccount? evmAccount.address : "no PK"}`); startTelegram(); startDiscord(); for (const w of getAllWallets()) await snapshotWallet(w); say.ok(`vigilancia activa - /add ALIAS WALLET RED MONTO (usa evm)`); setInterval(() => { loop().catch((e) => say.err(`ciclo SOL: ${e.message}`)); }, POLL_MS); setInterval(() => { loopEvm().catch((e) => say.err(`ciclo EVM: ${e.message}`)); }, POLL_EVM_MS); };
boot();
