/* MEMEBOT v3.3 — PnL REAL EVM + Heartbeat + 0x ready */
...
const POLL_EVM_MS = Math.max(5000, Number(process.env.POLL_EVM_MS || 15000));
...
const BANNER = `\n MEMEBOT v3.3 PnL REAL EVM + Heartbeat`;
function loadStateFile() { try { if (fs.existsSync(STATE_FILE)) { const s = JSON.parse(fs.readFileSync(STATE_FILE, "utf8")); if (s && typeof s.reserva === "number") { if (!s.customWallets) s.customWallets = []; if (!s.tgSubs) s.tgSubs = []; if (!s.evmWallets) s.evmWallets = {}; return s; } } } catch (e) { say.warn(`state.json corrupto (${e.message})`); } return { version: 3.3, reserva: RESERVA_INICIAL, usdc: 0, startedAt: Date.now(), positions: {}, evmPositions: {}, pnl: { solana: { pnlSol: 0, wins: 0, losses: 0, trades: 0, usdc: 0 }, base: { pnl: 0, wins: 0, losses: 0, trades: 0 }, bsc: { pnl: 0, wins: 0, losses: 0, trades: 0 }, robinhood: { pnl: 0, wins: 0, losses: 0, trades: 0 }, hyperevm: { pnl: 0, wins: 0, losses: 0, trades: 0 }, hyperliquid: { pnl: 0, wins: 0, losses: 0, trades: 0 } }, byWallet: {}, wallets: {}, tgSubs: [], customWallets: [], evmWallets: {} }; }
function migrateState(s){ if(!s.evmPositions) s.evmPositions={}; if(!s.pnl) s.pnl={ solana:{pnlSol:0,wins:0,losses:0,trades:0,usdc:0}, base:{pnl:0,wins:0,losses:0,trades:0}, bsc:{pnl:0,wins:0,losses:0,trades:0}, robinhood:{pnl:0,wins:0,losses:0,trades:0}, hyperevm:{pnl:0,wins:0,losses:0,trades:0}, hyperliquid:{pnl:0,wins:0,losses:0,trades:0}}; if(!s.byWallet) s.byWallet={}; for(const k of ["base","bsc","robinhood","hyperevm","hyperliquid","solana"]){ if(!s.pnl[k]) s.pnl[k]={pnl:0,pnlSol:0,wins:0,losses:0,trades:0,usdc:0}; } s.version=3.3; return s; }
...
async function getDexPrice(tokenAddress){
  ...
  try{
    ...
    if(price && price>0){ dexCache.set(key, { p: price, t: Date.now(), symbol }); return { price, symbol }; }
  }catch(e){}
  return null;
}
async function getEvmTokenFromTx(chain, txHash, wallet){
  try{
    if(chain==="base"){
      const { data } = await http.get(`https://api.basescan.org/api?module=account&action=tokentx&txhash=${txHash}&address=${wallet}`, { timeout: 8000 });
      if(data?.result?.[0]){ return { address: data.result[0].contractAddress, symbol: data.result[0].tokenSymbol }; }
    } else if(chain==="bsc"){
      const { data } = await http.get(`https://api.bscscan.com/api?module=account&action=tokentx&txhash=${txHash}&address=${wallet}`, { timeout: 8000 });
      if(data?.result?.[0]){ return { address: data.result[0].contractAddress, symbol: data.result[0].tokenSymbol }; }
    } else if(chain==="robinhood"){
      const { data } = await http.get(`https://robinhoodchain.blockscout.com/api/v2/transactions/${txHash}/token-transfers`, { timeout: 8000 });
      const t = data?.items?.[0]; if(t?.token?.address) return { address: t.token.address, symbol: t.token.symbol };
    } else if(chain==="hyperevm" || chain==="hyperliquid"){
      const { data } = await http.get(`https://www.hyperevmscan.io/api/v2/transactions/${txHash}/token-transfers`, { timeout: 8000 });
      const t = data?.items?.[0]; if(t?.token?.address) return { address: t.token.address, symbol: t.token.symbol };
    }
  }catch(e){}
  return null;
...
async function notify({ title, lines, color, hash, chain = "solana" }) { const explorer = chain === "solana"? solscan(hash) : evmExplorer(chain, hash); say.trade(`${title} ${C.d}${lines.map((l) => l.replace(/<[^>]+>/g, "")).join(" · ")}${C.x}`); if (tgBot) { const text = [`<b>${title}</b>`,...lines, hash? `🔗 <a href="${explorer}">ver en explorer</a>` : null].filter(Boolean).join("\n"); const targets = [...new Set([...state.tgSubs,...(TELEGRAM_CHAT_ID? [TELEGRAM_CHAT_ID] : [])])]; for (const chat of targets) { try { await tgBot.telegram.sendMessage(chat, text, { parse_mode: "HTML", disable_web_page_preview: true }); } catch {} } } if (discordReady && DISCORD_CHANNEL_ID) { try { const embed = new EmbedBuilder().setTitle(title).setDescription(lines.join("\n")).setColor(color).setTimestamp(); if (hash) embed.setURL(explorer); await discordClient.channels.cache.get(DISCORD_CHANNEL_ID)?.send({ embeds: [embed] }); } catch {} } }
...
async function pollEvmWallet(w) {
  ...
  try {
    let txList = [];
    if (chain === "base") { try { const { data } = await http.get(`https://api.basescan.org/api?module=account&action=txlist&address=${w.address}&startblock=0&endblock=99999999&sort=desc`); if (data.result && Array.isArray(data.result)) txList = data.result.slice(0, 5); } catch {} }
    else if (chain === "bsc") { try { const { data } = await http.get(`https://api.bscscan.com/api?module=account&action=txlist&address=${w.address}&sort=desc`); if (data.result && Array.isArray(data.result)) txList = data.result.slice(0, 5); } catch {} }
    else if (chain === "robinhood") { try { const { data } = await http.get(`https://robinhoodchain.blockscout.com/api/v2/addresses/${w.address}/transactions?items_count=5`); if (data.items) txList = data.items.map(it => ({ hash: it.hash, timeStamp: new Date(it.timestamp).getTime()/1000, input: it.raw_input })); } catch {} }
    else if (chain === "hyperevm" || chain === "hyperliquid") { try { const { data } = await http.get(`https://www.hyperevmscan.io/api/v2/addresses/${w.address}/transactions?items_count=5`); if (data.items) txList = data.items.map(it => ({ hash: it.hash, timeStamp: new Date(it.timestamp).getTime()/1000, input: it.raw_input })); } catch {} }
    for (const tx of txList) {
      const hash = tx.hash; if (!hash || seenEvmTx.has(hash)) continue;
      const tsTx = Number(tx.timeStamp || 0) * 1000; if (tsTx && tsTx <= ws.lastBlockTime) continue;
      seenEvmTx.add(hash);
      const isSwap = tx.input && tx.input!== "0x" && tx.input.length > 10;
      if (!isSwap) { ws.lastBlockTime = Math.max(ws.lastBlockTime, tsTx || Date.now()); continue; }
      ws.lastBlockTime = Math.max(ws.lastBlockTime, tsTx || Date.now()); ws.stats.copies++;
      // === v3.3 PRECIO REAL ===
      try{
        if(!state.evmPositions) state.evmPositions={};
        const tokenInfo = await getEvmTokenFromTx(chain, hash, w.address);
        let priceData = null;
        if(tokenInfo?.address) priceData = await getDexPrice(tokenInfo.address);
        const openKeys = Object.keys(state.evmPositions).filter(k=>k.startsWith(`${chain}:${w.address.toLowerCase()}:`) && state.evmPositions[k].token?.toLowerCase() === (tokenInfo?.address?.toLowerCase() || "unknown"));
        const hasOpen = openKeys.length>0 || Object.keys(state.evmPositions).filter(k=>k.startsWith(`${chain}:${w.address.toLowerCase()}:`)).length>0;
        // Si hay abierta, cerrar con PnL real
        const allOpenKeys = Object.keys(state.evmPositions).filter(k=>k.startsWith(`${chain}:${w.address.toLowerCase()}:`));
        if(allOpenKeys.length>0){
          const closeKey = allOpenKeys[0];
          const openPos = state.evmPositions[closeKey];
          ...
          delete state.evmPositions[closeKey];
          ...
          await notify({ title: `${pnlAmt>=0?'🟢':'🔴'} [${chain.toUpperCase()}] ${w.alias} VENDIÓ $${esc(tokenInfo?.symbol||openPos.symbol||'TOKEN')} — REAL`, lines: [`Token: <code>${(tokenInfo?.address||openPos.token||'').slice(0,10)}...</code>`, `Entrada: $${entryPrice?entryPrice.toFixed(6):'?.??'} → Salida: $${exitPrice?exitPrice.toFixed(6):'?.??'}`, `PnL: <b>${pnlPct>=0?'+':''}${pnlPct.toFixed(2)}%</b> (${pnlAmt>=0?'+':''}${pnlAmt.toFixed(6)} ${chain==='bsc'?'BNB':'ETH'})`, `Tx: <code>${hash.slice(0,12)}…</code>`], color: pnlAmt>=0?GREEN:RED, hash, chain });
        } else {
          state.evmPositions[`${chain}:${w.address.toLowerCase()}:${Date.now()}`]={wallet:w.address, alias:w.alias, chain, amount:w.tradeAmount, token: tokenInfo?.address||"unknown", symbol: tokenInfo?.symbol||priceData?.symbol||"UNKNOWN", entryPriceUsd: priceData?.price||0, openedAt:Date.now(), txHash:hash};
          await notify({ title: `🟢 [${chain.toUpperCase()}] ${w.alias} COMPRÓ $${esc(tokenInfo?.symbol||priceData?.symbol||'TOKEN')} — BOT ENTRÓ`, lines: [`Token: <code>${(tokenInfo?.address||'unknown').slice(0,12)}...</code>`, `Precio: $${priceData?.price?priceData.price.toFixed(6):'?.??'}`, `Monto: <b>${w.tradeAmount} ${chain==='bsc'?'BNB':'ETH'}</b>`, `Tx: <code>${hash.slice(0,12)}…</code>`], color: CYAN, hash, chain });
        }
      }catch(e){ say.warn(`PnL EVM real err: ${e.message}`); }
      await saveState();
    }
  } catch (e) { say.warn(`EVM poll ${chain} ${w.alias} falló: ${e.message}`); }
}
async function loopEvm() { lastEvmPoll = Date.now(); const evmChains = ["base", "bsc", "robinhood", "hyperevm"]; for (const chain of evmChains) { const wallets = getWalletsByChain(chain); for (const w of wallets) { await pollEvmWallet(w); await new Promise(r => setTimeout(r, 600)); } } }
const seenSigs = new Set();
async function pollWallet(w) { const ws = walletState(w.address, "solana"); let sigs; try { sigs = await rpc("getSignaturesForAddress", [w.address, { limit: 25 }]); } catch (e) { say.err(`RPC caído ${w.alias}: ${e.message}`); return; } const fresh = (sigs?? []).filter((s) =>!s.err &&!seenSigs.has(s.signature)).filter((s) => (s.blockTime?? 0) * 1000 > ws.lastBlockTime).sort((a, b) => (a.blockTime?? 0) - (b.blockTime?? 0)).slice(0, 10); for (const s of fresh) { seenSigs.add(s.signature); let tx = null; try { tx = await rpc("getTransaction", [s.signature, { encoding: "jsonParsed", maxSupportedTransactionVersion: 0 }]); } catch { continue; } if (!tx) continue; const ev = classify(tx, w.address); if (!ev) { ws.lastBlockTime = Math.max(ws.lastBlockTime, (tx.blockTime?? 0) * 1000); continue; } ws.lastBlockTime = Math.max(ws.lastBlockTime, ev.blockTime); try { await applyEvent(w, ev); } catch (e) { say.err(`procesando ${s.signature.slice(0, 12)}…: ${e.message}`); } } if (seenSigs.size > 2000) seenSigs.clear(); }
...
function expandChainInput(input){
  input=(input||"").toLowerCase().trim();
  if(!input) return ["solana"];
  if(input==="evm") return EVM_CHAINS;
  if(input==="all" || input==="todo" || input==="todas") return ALL_CHAINS;
  if(input.includes(",")){ const out=[]; for(const p of input.split(",")){ out.push(...expandChainInput(p)); } return [...new Set(out)]; }
  if(input==="hyperliquid") return ["hyperevm"];
  return [input];
}
let tgBot = null;
function parseAddCommand(text) {
  const parts = text.trim().split(/\s+/);
  if (parts.length < 2) return { error: "Uso: /add ALIAS WALLET RED MONTO" };
  const maybeWallet = parts[1];
  const isOldFormat = maybeWallet.length >= 32 || maybeWallet.startsWith("0x");
  let alias, wallet, chainPart, amountPart;
  if (isOldFormat) { wallet = parts[1]; alias = parts[2] || `${wallet.slice(0,4)}…${wallet.slice(-4)}`; const maybeAmount = parts[3]; return { wallets: [{ alias, address: wallet, chain: wallet.startsWith("0x")? "base" : "solana", tradeAmount: maybeAmount || (wallet.startsWith("0x")? DEFAULT_TRADE_EVM : String(DEFAULT_TRADE_SOL)) }], old: true }; }
  alias = parts[1]; wallet = parts[2]; chainPart = parts[3] || "solana"; amountPart = parts[4];
  if (!wallet) return { error: "Falta wallet. Uso: /add ALIAS WALLET RED MONTO" };
  const result = []; const expandedChains = expandChainInput(chainPart);
  for(const ch of expandedChains){ result.push({ alias, address: wallet, chain: ch.toLowerCase().trim(), tradeAmount: amountPart || (ch.toLowerCase()==="solana"? String(DEFAULT_TRADE_SOL) : DEFAULT_TRADE_EVM) }); }
  for (const w of result) { if (w.chain === "solana") { if (w.address.length < 32) return { error: `Wallet Solana invalida: ${w.address}` }; } else { if (!isAddress(w.address)) return { error: `Wallet EVM invalida: ${w.address}` }; } const validChains = ["solana", "base", "bsc", "robinhood", "hyperevm", "hyperliquid"]; if (!validChains.includes(w.chain)) return { error: `Chain invalida ${w.chain}. Usa: solana, base, bsc, robinhood, hyperevm o 'evm' para todas` }; }
  return { wallets: result };
}
let lastEvmPoll = Date.now(); let lastSolPoll = Date.now();
function heartbeat(){ const all = getAllWallets(); const openSol = Object.keys(state.positions||{}).length; const openEvm = Object.keys(state.evmPositions||{}).length; const pnl = state.pnl||{}; const solPnl = pnl.solana?.pnlSol||0; console.log(`${new Date().toLocaleTimeString()} 💓 heartbeat | wallets: ${all.length} (SOL:${getWalletsByChain("solana").length} BASE:${getWalletsByChain("base").length} BSC:${getWalletsByChain("bsc").length} ROB:${getWalletsByChain("robinhood").length} HYPE:${getWalletsByChain("hyperevm").length}) | pos: SOL ${openSol} EVM ${openEvm} | reserva ${state.reserva.toFixed(4)} SOL | PnL SOL ${solPnl>=0?"+":""}${solPnl.toFixed(4)} | last SOL ${((Date.now()-lastSolPoll)/1000|0)}s ago EVM ${((Date.now()-lastEvmPoll)/1000|0)}s ago`); }
const boot = async () => { await initDB(); if (pool) { const dbState = await loadStateDB(); if (dbState) { state = migrateState(dbState); say.ok(`DB: PostgreSQL Connected ✅ - ${state.customWallets.length} wallets`); } } say.info(`RPC SOL: ${HELIUS_API_KEY? "Helius" : "publico"}`); say.info(`RPC EVM: BASE ${BASE_RPC} | BSC ${BSC_RPC} | ROBINHOOD ${ROBINHOOD_RPC} | HYPERLIQUID ${HYPERLIQUID_RPC}`); say.info(`Wallets: ${getAllWallets().length} total`); say.info(`EVM account: ${evmAccount? evmAccount.address : "no PK"}`); startTelegram(); startDiscord(); for (const w of getAllWallets()) await snapshotWallet(w); say.ok(`vigilancia activa v3.3 REAL`); setInterval(() => { loop().catch((e) => say.err(`ciclo SOL: ${e.message}`)); }, POLL_MS); setInterval(() => { loopEvm().catch((e) => say.err(`ciclo EVM: ${e.message}`)); }, POLL_EVM_MS); setInterval(() => { heartbeat(); }, 5*60*1000); setTimeout(()=>heartbeat(), 10000); };
async function startTelegram() { if (!TELEGRAM_TOKEN) { say.warn("sin TELEGRAM_BOT_TOKEN"); return; } tgBot = new Telegraf(TELEGRAM_TOKEN); tgBot.command("start", async (ctx) => { if (!state.tgSubs.includes(ctx.chat.id)) { state.tgSubs.push(ctx.chat.id); await saveState(); } const dbStatus = pool? "PostgreSQL Connected ✅" : "Local file"; ctx.reply(`🟢 <b>MEMEBOT v3.3 REAL PnL</b> - DB: ${dbStatus}\nChains: solana, base, bsc, robinhood, hyperevm\n/estado · /pos · /wallets · /pnl · /top · /heartbeat`, { parse_mode: "HTML" }); }); tgBot.command("help", (ctx) => ctx.reply("<b>Comandos v3.3 REAL</b>\n/estado\n/pos\n/wallets\n/pnl [ALIAS]\n/top\n/heartbeat\n/add ALIAS WALLET RED MONTO", { parse_mode: "HTML" })); tgBot.command("add", async (ctx) => { const parsed = parseAddCommand(ctx.message.text); if (parsed.error) return ctx.reply(parsed.error); const added = []; for (const nw of parsed.wallets) { const exists = getAllWallets().some(w => w.address.toLowerCase() === nw.address.toLowerCase() && (w.chain||"solana").toLowerCase() === nw.chain.toLowerCase()); if (exists) { await ctx.reply(`⚠️ Ya existe ${nw.alias} en ${nw.chain}`); continue; } const obj = { address: nw.address, alias: nw.alias, chain: nw.chain, tradeAmount: String(nw.tradeAmount), type: nw.chain === "solana"? "solana" : "evm" }; state.customWallets.push(obj); added.push(obj); await snapshotWallet(obj); } await saveState(); if (added.length) { ctx.reply(`✅ Agregadas ${added.length}:\n` + added.map(a => `• <b>${esc(a.alias)}</b> ${a.chain} — <code>${a.address.slice(0,6)}…${a.address.slice(-4)}</code> — ${a.tradeAmount}`).join("\n"), { parse_mode: "HTML" }); } }); tgBot.command("remove", async (ctx) => { const parts = ctx.message.text.trim().split(/\s+/); const target = parts[1]; if (!target) return ctx.reply("Uso: /remove ALIAS o DIRECCION"); const before = state.customWallets.length; const chainFilter = parts[2]; if (chainFilter) { state.customWallets = state.customWallets.filter(w =>!((w.alias===target || w.address.toLowerCase()===target.toLowerCase()) && w.chain===chainFilter.toLowerCase())); } else { state.customWallets = state.customWallets.filter(w => w.address.toLowerCase()!== target.toLowerCase() && w.alias!== target); } if (before === state.customWallets.length) return ctx.reply(`No encontré ${target}`); await saveState(); ctx.reply(`🗑️ Eliminada: ${esc(target)}`); }); tgBot.command("estado", async (ctx) => { const solUsd = await getSolUsd(); const dbInfo = pool? "🐘 Postgres" : "📁 Archivo"; const evmInfo = evmAccount? `EVM: ${evmAccount.address.slice(0,6)}…` : "EVM: solo lectura"; ctx.reply(`<b>📊 ESTADO v3.3 REAL - ${dbInfo}</b>\nReserva: <b>${state.reserva.toFixed(4)} SOL</b>${usd(state.reserva, solUsd)}\n${evmInfo}\nWallets: ${getAllWallets().length} total — SOL:${getWalletsByChain('solana').length} BASE:${getWalletsByChain('base').length} BSC:${getWalletsByChain('bsc').length} ROB:${getWalletsByChain('robinhood').length} HYPE:${getWalletsByChain('hyperevm').length}\nPos: SOL ${Object.keys(state.positions||{}).length} EVM ${Object.keys(state.evmPositions||{}).length}`, { parse_mode: "HTML" }); }); tgBot.command("pos", (ctx) => { const openSol = Object.values(state.positions||{}); const openEvm = Object.values(state.evmPositions||{}); if (!openSol.length &&!openEvm.length) return ctx.reply("Sin posiciones."); let msg=""; if(openSol.length) msg+= "<b>SOL:</b>\n"+ openSol.map((p) => `• <b>$${esc(p.symbol)}</b> — ${p.amountSol.toFixed(4)} SOL`).join("\n")+"\n\n"; if(openEvm.length) msg+= "<b>EVM REAL:</b>\n"+ openEvm.map((p)=>`• ${p.chain.toUpperCase()} ${esc(p.alias)} $${esc(p.symbol)} @ $${p.entryPriceUsd?.toFixed(6)||'?'} — ${p.amount} ${p.chain==="bsc"?"BNB":"ETH"}`).join("\n"); ctx.reply(msg, { parse_mode: "HTML" }); });
  tgBot.command("pnl", async (ctx) => { const parts = ctx.message.text.trim().split(/\s+/); const filter = parts[1]; const pnl = state.pnl||{}; const byWallet = state.byWallet||{}; if(filter){ const list = Object.values(byWallet).filter(v=>v.alias.toLowerCase()===filter.toLowerCase()); if(!list.length) return ctx.reply(`Sin PnL para ${filter}`); let msg = `<b>📊 PnL ${esc(filter)} REAL</b>\n`; for(const w of list){ msg+=`• ${w.chain.toUpperCase()}: ${w.pnl>=0?"+":""}${w.pnl.toFixed(6)} | W:${w.wins} L:${w.losses} T:${w.trades} ${w.pnl>=0?"🟢":"🔴"}\n`; } const total = list.reduce((a,b)=>a+b.pnl,0); msg+=`\n<b>Total: ${total>=0?"+":""}${total.toFixed(6)}</b>`; return ctx.reply(msg, {parse_mode:"HTML"}); } let msg = `<b>📊 PNL GLOBAL v3.3 REAL</b>\n`; msg+=`SOL: ${pnl.solana?.pnlSol>=0?"+":""}${(pnl.solana?.pnlSol||0).toFixed(4)} SOL | W:${pnl.solana?.wins||0} L:${pnl.solana?.losses||0} T:${pnl.solana?.trades||0} | USDC $${(pnl.solana?.usdc||0).toFixed(2)}\n`; for(const ch of ["base","bsc","robinhood","hyperevm"]){ const d=pnl[ch]; if(!d) continue; msg+=`${ch.toUpperCase()}: ${d.pnl>=0?"+":""}${d.pnl.toFixed(6)} ${ch==="bsc"?"BNB":"ETH"} | W:${d.wins} L:${d.losses} T:${d.trades} ${d.pnl>=0?"🟢":"🔴"}\n`; } msg+=`\nReserva: ${state.reserva.toFixed(4)} SOL\nPos: SOL ${Object.keys(state.positions||{}).length} EVM ${Object.keys(state.evmPositions||{}).length}`; ctx.reply(msg, {parse_mode:"HTML"}); });
  tgBot.command("top", (ctx) => { const byWallet = Object.values(state.byWallet||{}); if(!byWallet.length) return ctx.reply("Aún sin trades."); const sorted = byWallet.sort((a,b)=>b.pnl-a.pnl).slice(0,10); let msg = `<b>🏆 TOP WALLETS REAL</b>\n`; sorted.forEach((w,i)=>{ msg+=`${i+1}. <b>${esc(w.alias)}</b> [${w.chain}] ${w.pnl>=0?"+":""}${w.pnl.toFixed(6)} W:${w.wins} L:${w.losses}\n`; }); ctx.reply(msg, {parse_mode:"HTML"}); });
  tgBot.command("heartbeat", (ctx)=>{ heartbeat(); ctx.reply("💓 heartbeat enviado a logs"); });
  tgBot.command("wallets", (ctx) => { const all = getAllWallets(); if (!all.length) return ctx.reply("Sin wallets."); const byChain = {}; for (const w of all) { const c = w.chain || "solana"; if (!byChain[c]) byChain[c]=[]; byChain[c].push(w); } let msg = ""; for (const [chain, list] of Object.entries(byChain)) { msg += `<b>${chain.toUpperCase()} (${list.length}):</b>\n`; msg += list.map((w) => `• <b>${esc(w.alias)}</b> — ${w.tradeAmount} · ${w.address.slice(0,6)}…`).join("\n") + "\n\n"; } ctx.reply(msg, { parse_mode: "HTML" }); }); tgBot.command("reset", async (ctx) => { state.reserva = RESERVA_INICIAL; state.usdc = 0; state.positions = {}; state.evmPositions={}; state.pnl={ solana: { pnlSol: 0, wins: 0, losses: 0, trades: 0, usdc: 0 }, base: { pnl: 0, wins: 0, losses: 0, trades: 0 }, bsc: { pnl: 0, wins: 0, losses: 0, trades: 0 }, robinhood: { pnl: 0, wins: 0, losses: 0, trades: 0 }, hyperevm: { pnl: 0, wins: 0, losses: 0, trades: 0 }, hyperliquid: { pnl: 0, wins: 0, losses: 0, trades: 0 } }; state.byWallet={}; for (const a of Object.keys(state.wallets)) { state.wallets[a].stats = { copies: 0, ignored: 0, dust: 0, pnlSol: 0, usdcSecured: 0 }; } await saveState(); ctx.reply(`🔄 Tesorería reiniciada: ${RESERVA_INICIAL} SOL + PnL REAL limpio`); }); try { await tgBot.telegram.deleteWebhook({ drop_pending_updates: true }); } catch {} try { const t = tgBot.token; if (t) await axios.get('https://api.telegram.org/bot' + t + '/deleteWebhook?drop_pending_updates=true'); say.ok("Webhook limpiado"); } catch {} await new Promise(r => setTimeout(r, 2000)); tgBot.launch({ dropPendingUpdates: true }).then(() => say.ok("Telegram v3.3 REAL ✅")); tgBot.catch((e) => say.err(`telegram: ${e.message}`)); }
