/* MEMEBOT v3.4.3 — FIX Too Many Requests Robinhood - block cache + throttling */
...
const POLL_EVM_MS = Math.max(15000, Number(process.env.POLL_EVM_MS || 20000));
...
const BANNER = `\n MEMEBOT v3.4.3 TooManyRequests FIX`;
function loadStateFile() { try { if (fs.existsSync(STATE_FILE)) { const s = JSON.parse(fs.readFileSync(STATE_FILE, "utf8")); if (s && typeof s.reserva === "number") { if (!s.customWallets) s.customWallets = []; if (!s.tgSubs) s.tgSubs = []; if (!s.evmWallets) s.evmWallets = {}; return s; } } } catch (e) { say.warn(`state.json corrupto (${e.message})`); } return { version: 3.43, reserva: RESERVA_INICIAL, usdc: 0, startedAt: Date.now(), positions: {}, evmPositions: {}, pnl: { solana: { pnlSol: 0, wins: 0, losses: 0, trades: 0, usdc: 0 }, base: { pnl: 0, wins: 0, losses: 0, trades: 0 }, bsc: { pnl: 0, wins: 0, losses: 0, trades: 0 }, robinhood: { pnl: 0, wins: 0, losses: 0, trades: 0 }, hyperevm: { pnl: 0, wins: 0, losses: 0, trades: 0 }, hyperliquid: { pnl: 0, wins: 0, losses: 0, trades: 0 } }, byWallet: {}, wallets: {}, tgSubs: [], customWallets: [], evmWallets: {} }; }
function migrateState(s){ if(!s.evmPositions) s.evmPositions={}; if(!s.pnl) s.pnl={ solana:{pnlSol:0,wins:0,losses:0,trades:0,usdc:0}, base:{pnl:0,wins:0,losses:0,trades:0}, bsc:{pnl:0,wins:0,losses:0,trades:0}, robinhood:{pnl:0,wins:0,losses:0,trades:0}, hyperevm:{pnl:0,wins:0,losses:0,trades:0}, hyperliquid:{pnl:0,wins:0,losses:0,trades:0}}; if(!s.byWallet) s.byWallet={}; for(const k of ["base","bsc","robinhood","hyperevm","hyperliquid","solana"]){ if(!s.pnl[k]) s.pnl[k]={pnl:0,pnlSol:0,wins:0,losses:0,trades:0,usdc:0}; } s.version=3.43; return s; }
...
let cachedRobinhoodBlock = 0n;
let cachedRobinhoodBlockTime = 0;

async function getRobinhoodBlockCached(){
  if(cachedRobinhoodBlock && Date.now() - cachedRobinhoodBlockTime < 15000) return cachedRobinhoodBlock;
  try{
    const b = await clients.robinhood.getBlockNumber();
    cachedRobinhoodBlock = b;
    cachedRobinhoodBlockTime = Date.now();
    return b;
  }catch(e){
    if(cachedRobinhoodBlock) return cachedRobinhoodBlock;
    throw e;
  }
}

async function pollEvmWallet(w, sharedBlock) {
  ...
  try {
    ...
    if (chain === "robinhood") {
      try {
        const currentBlock = sharedBlock || await getRobinhoodBlockCached();
        const fromBlock = lastRobinhoodBlock[w.address] || (currentBlock - 200n);
        if(fromBlock >= currentBlock) { lastRobinhoodBlock[w.address]=currentBlock; return; }
        const walletTopic = `0x000000000000000000000000${w.address.slice(2).toLowerCase()}`;
        const transferSig = "0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef";
        // solo 2 calls pequeñas por wallet, y ya no pide blockNumber
        const [logsBuy, logsSell] = await Promise.all([
          clients.robinhood.getLogs({ fromBlock, toBlock: currentBlock, topics: [transferSig, null, walletTopic] }).catch(()=>[]),
          clients.robinhood.getLogs({ fromBlock, toBlock: currentBlock, topics: [transferSig, walletTopic, null] }).catch(()=>[])
        ]);
        if(logsBuy.length||logsSell.length) console.log(`[ROBINHOOD RPC] ${w.alias} ${fromBlock}->${currentBlock} b:${logsBuy.length} s:${logsSell.length}`);
        transfers = [...logsBuy,...logsSell].map(l=>({ hash: l.transactionHash, timeStamp: Math.floor(Date.now()/1000), token: l.address, symbol: "TOKEN", isBuy: l.topics[2]?.toLowerCase().endsWith(w.address.slice(2).toLowerCase()) }));
        lastRobinhoodBlock[w.address] = currentBlock;
      } catch(e){
        const msg = e.message||"";
        if(msg.includes("Too Many")) console.log(`[ROB RATE LIMIT] esperando...`);
        else console.log(`[ROB RPC ERR] ${w.alias} ${msg.slice(0,120)}`);
      }
    }
    else if (chain === "base") {
      try { const { data } = await http.get(`https://base.blockscout.com/api/v2/addresses/${w.address}/token-transfers?items_count=15`, {timeout:8000}); if(data.items) transfers = data.items.map(it=>({ hash: it.transaction_hash, timeStamp: Math.floor(new Date(it.timestamp).getTime()/1000), token: it.token?.address, symbol: it.token?.symbol, isBuy: it.to?.hash?.toLowerCase()===w.address.toLowerCase() })); } catch {}
    }
    else if (chain === "bsc") {
      try { const {data}=await http.get(`https://api.bscscan.com/api?module=account&action=tokentx&address=${w.address}&sort=desc`, {timeout:8000}); if(data.result) transfers=data.result.slice(0,10).map(it=>({hash:it.hash,timeStamp:Number(it.timeStamp),token:it.contractAddress,symbol:it.tokenSymbol,isBuy:it.to?.toLowerCase()===w.address.toLowerCase()})); } catch {}
    }
    else if (chain === "hyperevm" || chain === "hyperliquid") {
      try { const { data } = await http.get(`https://www.hyperevmscan.io/api/v2/addresses/${w.address}/token-transfers?items_count=15`, {timeout:8000}); if(data.items) transfers = data.items.map(it=>({ hash: it.transaction_hash, timeStamp: Math.floor(new Date(it.timestamp).getTime()/1000), token: it.token?.address, symbol: it.token?.symbol, isBuy: it.to?.hash?.toLowerCase()===w.address.toLowerCase() })); } catch {}
    }
...
async function loopEvm() {
  lastEvmPoll = Date.now();
  let robinhoodBlock = null;
  try{ robinhoodBlock = await getRobinhoodBlockCached(); }catch{}
  const evmChains = ["base", "bsc", "robinhood", "hyperevm"];
  for (const chain of evmChains) {
    const wallets = getWalletsByChain(chain);
    for (const w of wallets) {
      await pollEvmWallet(w, robinhoodBlock);
      await new Promise(r => setTimeout(r, chain==="robinhood"? 800 : 400));
    }
  }
}
...
async function startTelegram() { if (!TELEGRAM_TOKEN) { say.warn("sin TELEGRAM_BOT_TOKEN"); return; } tgBot = new Telegraf(TELEGRAM_TOKEN); tgBot.command("start", async (ctx) => { if (!state.tgSubs.includes(ctx.chat.id)) { state.tgSubs.push(ctx.chat.id); await saveState(); } const dbStatus = pool? "PostgreSQL Connected ✅" : "Local file"; ctx.reply(`🟢 <b>MEMEBOT v3.4.3</b> - DB: ${dbStatus}\nChains: solana, base, bsc, robinhood, hyperevm\n/estado · /pos · /wallets · /pnl · /top · /heartbeat`, { parse_mode: "HTML" }); }); tgBot.command("help", (ctx) => ctx.reply("<b>Comandos v3.4.3</b>\n/estado\n/pos\n/wallets\n/pnl [ALIAS]\n/top\n/heartbeat\n/add ALIAS WALLET RED MONTO", { parse_mode: "HTML" })); tgBot.command("add", async (ctx) => { const parsed = parseAddCommand(ctx.message.text); if (parsed.error) return ctx.reply(parsed.error); const added = []; for (const nw of parsed.wallets) { const exists = getAllWallets().some(w => w.address.toLowerCase() === nw.address.toLowerCase() && (w.chain||"solana").toLowerCase() === nw.chain.toLowerCase()); if (exists) { await ctx.reply(`⚠️ Ya existe ${nw.alias} en ${nw.chain}`); continue; } const obj = { address: nw.address, alias: nw.alias, chain: nw.chain, tradeAmount: String(nw.tradeAmount), type: nw.chain === "solana"? "solana" : "evm" }; state.customWallets.push(obj); added.push(obj); await snapshotWallet(obj); } await saveState(); if (added.length) { ctx.reply(`✅ Agregadas ${added.length}:\n` + added.map(a => `• <b>${esc(a.alias)}</b> ${a.chain} — <code>${a.address.slice(0,6)}…${a.address.slice(-4)}</code> — ${a.tradeAmount}`).join("\n"), { parse_mode: "HTML" }); } }); tgBot.command("remove", async (ctx) => { const parts = ctx.message.text.trim().split(/\s+/); const target = parts[1]; if (!target) return ctx.reply("Uso: /remove ALIAS o DIRECCION"); const before = state.customWallets.length; const chainFilter = parts[2]; if (chainFilter) { state.customWallets = state.customWallets.filter(w =>!((w.alias===target || w.address.toLowerCase()===target.toLowerCase()) && w.chain===chainFilter.toLowerCase())); } else { state.customWallets = state.customWallets.filter(w => w.address.toLowerCase()!== target.toLowerCase() && w.alias!== target); } if (before === state.customWallets.length) return ctx.reply(`No encontré ${target}`); await saveState(); ctx.reply(`🗑️ Eliminada: ${esc(target)}`); }); tgBot.command("estado", async (ctx) => { const solUsd = await getSolUsd(); const dbInfo = pool? "🐘 Postgres" : "📁 Archivo"; const evmInfo = evmAccount? `EVM: ${evmAccount.address.slice(0,6)}…` : "EVM: solo lectura"; ctx.reply(`<b>📊 ESTADO v3.4.3 - ${dbInfo}</b>\nReserva: <b>${state.reserva.toFixed(4)} SOL</b>${usd(state.reserva, solUsd)}\n${evmInfo}\nWallets: ${getAllWallets().length} total — SOL:${getWalletsByChain('solana').length} BASE:${getWalletsByChain('base').length} BSC:${getWalletsByChain('bsc').length} ROB:${getWalletsByChain('robinhood').length} HYPE:${getWalletsByChain('hyperevm').length}\nPos: SOL ${Object.keys(state.positions||{}).length} EVM ${Object.keys(state.evmPositions||{}).length}`, { parse_mode: "HTML" }); }); tgBot.command("pos", (ctx) => { const openSol = Object.values(state.positions||{}); const openEvm = Object.values(state.evmPositions||{}); if (!openSol.length &&!openEvm.length) return ctx.reply("Sin posiciones."); let msg=""; if(openSol.length) msg+= "<b>SOL:</b>\n"+ openSol.map((p) => `• <b>$${esc(p.symbol)}</b> — ${p.amountSol.toFixed(4)} SOL`).join("\n")+"\n\n"; if(openEvm.length) msg+= "<b>EVM:</b>\n"+ openEvm.map((p)=>`• ${p.chain.toUpperCase()} ${esc(p.alias)} $${esc(p.symbol)} @ $${p.entryPriceUsd?.toFixed(6)||'?'} — ${p.amount} ${p.chain==="bsc"?"BNB":"ETH"}`).join("\n"); ctx.reply(msg, { parse_mode: "HTML" }); });
  ...
  tgBot.command("wallets", (ctx) => { const all = getAllWallets(); if (!all.length) return ctx.reply("Sin wallets."); const byChain = {}; for (const w of all) { const c = w.chain || "solana"; if (!byChain[c]) byChain[c]=[]; byChain[c].push(w); } let msg = ""; for (const [chain, list] of Object.entries(byChain)) { msg += `<b>${chain.toUpperCase()} (${list.length}):</b>\n`; msg += list.map((w) => `• <b>${esc(w.alias)}</b> — ${w.tradeAmount} · ${w.address.slice(0,6)}…`).join("\n") + "\n\n"; } ctx.reply(msg, { parse_mode: "HTML" }); }); tgBot.command("reset", async (ctx) => { state.reserva = RESERVA_INICIAL; state.usdc = 0; state.positions = {}; state.evmPositions={}; state.pnl={ solana: { pnlSol: 0, wins: 0, losses: 0, trades: 0, usdc: 0 }, base: { pnl: 0, wins: 0, losses: 0, trades: 0 }, bsc: { pnl: 0, wins: 0, losses: 0, trades: 0 }, robinhood: { pnl: 0, wins: 0, losses: 0, trades: 0 }, hyperevm: { pnl: 0, wins: 0, losses: 0, trades: 0 }, hyperliquid: { pnl: 0, wins: 0, losses: 0, trades: 0 } }; state.byWallet={}; for (const a of Object.keys(state.wallets)) { state.wallets[a].stats = { copies: 0, ignored: 0, dust: 0, pnlSol: 0, usdcSecured: 0 }; } await saveState(); ctx.reply(`🔄 Tesorería reiniciada: ${RESERVA_INICIAL} SOL + PnL limpio`); }); try { await tgBot.telegram.deleteWebhook({ drop_pending_updates: true }); } catch {} try { const t = tgBot.token; if (t) await axios.get('https://api.telegram.org/bot' + t + '/deleteWebhook?drop_pending_updates=true'); say.ok("Webhook limpiado"); } catch {} await new Promise(r => setTimeout(r, 2000)); tgBot.launch({ dropPendingUpdates: true }).then(() => say.ok("Telegram v3.4.3 ✅")); tgBot.catch((e) => say.err(`telegram: ${e.message}`)); }
